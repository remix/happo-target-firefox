...
exports['default'] = removeScrollbars;
...
// hidden.
function removeScrollbars(node) {
  var isOverflowing = node.scrollHeight !== node.clientHeight || node.scrollWidth !== node.clientWidth;
  ...
  if (isAutoOrScroll(style.getPropertyValue('overflow-y')) || isAutoOrScroll(style.getPropertyValue('overflow-x')) || isAutoOrScroll(style.getPropertyValue('overflow'))) {
