...
exports['default'] = getFullRect;

var _removeScrollbars = require('./removeScrollbars');

var _removeScrollbars2 = _interopRequireDefault(_removeScrollbars);
...
function getFullRectRecursive(node, box) {
  ...
  (0, _removeScrollbars2['default'])(node);
...
function getFullRect(rootNodes) {
  var boxes = [];
  ...
  for (var i = 0; i < rootNodes.length; i += 1) {
    ...
    getFullRectRecursive(node, _box);
    ...
    _box.bottom += parseFloat(computedStyle.getPropertyValue('margin-bottom') || 0);
    _box.left -= parseFloat(computedStyle.getPropertyValue('margin-left') || 0);
    _box.right += parseFloat(computedStyle.getPropertyValue('margin-right') || 0);
    _box.top -= parseFloat(computedStyle.getPropertyValue('margin-top') || 0);

    boxes.push(_box);
  }

  // Merge all boxes together.
  var box = boxes.reduce(function (a, b) {
    mergeBoxes(a, b);
    return a;
  }, {
    bottom: 0,
